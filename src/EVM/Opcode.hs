...
-- > OpSpec 0x61 0 1 "push2 256"
data OpcodeSpecification = OpSpec
  { opcodeEncoding :: Word8 -- ^ The numeric encoding of this opcode
  , opcodeAlpha    :: Word8 -- ^ The number of items that this opcode places on the stack (α)
  , opcodeDelta    :: Word8 -- ^ The number of items that this opcode removes from the stack (δ)
...
opcodeSpec opcode = case opcode of
  ...
  PUSH n ->
    ...
    in OpSpec { opcodeEncoding = pushHex
              , opcodeAlpha    = 0
              , opcodeDelta    = 1
  ...
  DUP i -> OpSpec { opcodeEncoding = 0x80 + fromIntegral i
                  , opcodeAlpha    = fromIntegral i + 1
                  , opcodeDelta    = fromIntegral i + 2
                  , opcodeName     = "dup" <> Text.pack (show (i+1))
                  }

  -- 90s: Exchange operations (SWAP)
  SWAP i -> OpSpec { opcodeEncoding = 0x90 + fromIntegral i
                   , opcodeAlpha    = fromIntegral i + 1
                   , opcodeDelta    = fromIntegral i + 1
                   , opcodeName     = "swap" <> Text.pack (show (i+1))
                   }

  -- a0s: Logging Operations (LOG)
  LOG i -> OpSpec { opcodeEncoding = 0xa0 + fromIntegral i
                  , opcodeAlpha    = fromIntegral i + 2
                  , opcodeDelta    = 0
                  , opcodeName     = "log" <> Text.pack (show (i+1))
                  }
...
readOp b bs
  | isDUP b   = pure . DUP  . fromIntegral $ b - 0x80
  | isSWAP b  = pure . SWAP . fromIntegral $ b - 0x90
  | isLOG b   = pure . LOG  . fromIntegral $ b - 0xa0
